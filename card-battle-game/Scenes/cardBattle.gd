...
func pick_card(deck: Array, index := 0) -> Card:
	if deck.is_empty():
		return null
	if index < 0 or index >= deck.size():
		return null
	return deck.remove_at(index)
...
func play_turn(player_indexes: Array) -> Dictionary:
	# 1. Enemy draws
	enemy_hand = get_enemy_hand(enemy_deck, enemy_discard, 3)

	# 2. Player plays cards
	var chosen_cards = []
	for idx in player_indexes:
		var c = pick_card(player_hand, player_discard, idx)
	...
	for c in chosen_cards:
		if c: player_discard.append(c)

	for c in enemy_hand:
		if c: enemy_discard.append(c)

	# 5. Refill hand to minimum 5
	refill_player_hand()
